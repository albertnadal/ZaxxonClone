...
int Position::GetCellX() const {
    return 1; //(int_x + (CELL_WIDTH >> 1)) / CELL_WIDTH;
}

int Position::GetInitialCellX() const {
    return 1; //(initial_int_x + (CELL_WIDTH >> 1)) / CELL_WIDTH;
}

int Position::GetCellY() const {
    return 1; //(int_y + (CELL_HEIGHT >> 1)) / CELL_HEIGHT;
}

int Position::GetInitialCellY() const {
    return 1; //(initial_int_y + (CELL_HEIGHT >> 1)) / CELL_HEIGHT;
}
